...
The training-loss sweep adds one nuance to the perplexity result. The within-optimizer sweep loss curves are shown in Figures~\ref{fig:sweep-losses-adam} and~\ref{fig:sweep-losses-muon}, with the corresponding advantage heatmaps in Figures~\ref{fig:heatmap-adam} and~\ref{fig:heatmap-muon}. In the Adam comparison, AdamH is most competitive in a mid-range learning-rate window, whereas AdamW is more stable at the ends (Figures~\ref{fig:sweep-losses-adam} and~\ref{fig:heatmap-adam}). In the Muon comparison, MuonH is competitive at smaller learning rates, while MuonW improves toward the upper end (Figures~\ref{fig:sweep-losses-muon} and~\ref{fig:heatmap-muon}). MuonW's lowest-loss region is closer to $\sim 5.69 \times 10^{-3}$ than to the perplexity-selected $\sim 2.27 \times 10^{-2}$, but the two MuonW training losses are nearly tied. We therefore treat training loss as a diagnostic rather than the selection rule: the main claim is about held-out perplexity, FLOPs-to-target, and Wall-Clock time-to-target, while this loss view only shows that the larger MuonW learning rate does not buy its perplexity result by obviously dominating every training-loss slice.

Across optimizers, the same pattern becomes more pronounced with width. MuonW and AdamH are relatively close at smaller widths and lower-to-mid learning rates, but MuonW gains a clearer advantage at higher learning rates and larger widths (Figures~\ref{fig:cross-optimizer-heatmap} and~\ref{fig:muonw-adamh-sweep-loss}). This is the useful interpretation of the learning-rate sweep: Muon's best region shifts upward in learning rate, and that high-learning-rate region becomes more valuable as scale increases. This scaling trend is important because optimizer choice matters more, not less, as the model moves toward the regime where training compute dominates iteration cost.
...
Consistent with the norm-control mechanism in Section~\ref{sec:norm-control-ranking}, MuonH has lower weight-spectrum diversity than MuonW. The update spectra help separate optimizer behavior from the accumulated weight spectrum: MuonW and MuonH both maintain high update SE, and their update PR remain much larger than those of AdamW and AdamH (Figure~\ref{fig:spectral}). Thus, Hyperball does not appear to eliminate Muon's matrix-whitened update structure. The main difference is instead that the fixed-radius weight constraint yields a lower-diversity weight spectrum than MuonW, worsening model performance, while AdamH shows the strongest concentration among the configurations shown. MuonW has no hard rescaling step and maintains the highest participation ratio among the weight matrices shown: independent weight decay changes the weight matrix that the Muon update is added to, but it does not reproject the post-update matrix onto a fixed-radius sphere. This makes the regularizer less directly coupled to Muon's update whitening.
...
\begin{figure}[htbp]
  ...
  \caption{Zipf's law. The left panel shows Zipf's law on a linear scale, and the right panel shows it on a log-log scale. The straight line on the log-log plot is the signature of a power-law distribution. Source: Statology~\citep{zipf}.}
  \label{fig:zipf}
\end{figure}
