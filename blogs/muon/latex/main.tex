...
\title{Muon Reduces Training Cost of Regulatory DNA Transformers}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={draw, align=center, minimum height=0.9cm, inner xsep=9pt, inner ysep=7pt, font=\small},
    inputbox/.style={box, fill=green!8, draw=green!45!black, minimum width=3.4cm},
    model/.style={box, fill=blue!6, draw=blue!50!black},
    stack/.style={box, fill=violet!8, draw=violet!65!black, minimum width=3.7cm, minimum height=2.15cm},
    head/.style={box, fill=teal!7, draw=teal!55!black},
    data/.style={box, fill=orange!7, draw=orange!55!black},
    arrow/.style={-{Latex[length=2mm]}, thick}
  ]
    \node[inputbox] (input) {\begin{tabular}{c}
      \textbf{Nucleotide input}\\[-1pt]
      {\scriptsize\fbox{\texttt{A}}\;\fbox{\texttt{C}}\;\fbox{\texttt{G}}\;\fbox{\texttt{T}}\;\fbox{\texttt{[MASK]}}\;\fbox{\texttt{G}}\;\fbox{\texttt{A}}}\\[-1pt]
      {\scriptsize real cCRE DNA tokens}
    \end{tabular}};
    \node[model, right=of input, minimum width=2.0cm] (embed) {\textbf{Embedding}\\token + position};
    \node[stack, right=of embed] (encoder) {\textbf{Transformer encoder stack}\\[2pt]non-causal self-attention\\RoPE + SwiGLU FFN};
    \node[head, right=1.1cm of encoder, yshift=0.65cm] (mlm) {\textbf{Nucleotide head}\\MLM logits};
    \node[head, right=1.1cm of encoder, yshift=-0.65cm] (modality) {\textbf{Modality head}\\DNase-seq tracks};
...
Figure~\ref{fig:scale-advantages} summarizes this matched-target comparison across model scales using the median Muon improvement. The plot reports a median Muon improvement across all scales, suggesting this is a trend that would apply to models of all sizes.
...
The sweep loss curves and advantage heatmaps are shown in Figures~\ref{fig:sweep-losses-adam}--\ref{fig:cross-optimizer-heatmap}. AdamH is most competitive in a mid-range learning-rate window, whereas AdamW is more stable at the ends. For Muon, MuonH is competitive at smaller learning rates, while MuonW improves toward the upper end.
...
Consistent with the norm-control mechanism in Section~\ref{sec:norm-control-ranking}, MuonH has lower weight-spectrum diversity than MuonW. However, Figure~\ref{fig:spectral} suggests that it preserves the high-diversity Muon update spectrum, since it still has higher PR and SE than Adam. MuonW has no hard rescaling step and maintains the highest participation ratio among the weight matrices shown: independent weight decay changes the weight matrix that the Muon update is added to, but it does not reproject the post-update matrix onto a fixed-radius sphere. This makes the regularizer less directly coupled to Muon's update whitening.
