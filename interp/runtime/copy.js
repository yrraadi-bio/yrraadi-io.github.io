import { CARD_LIMIT, blockNumber, signed } from "./core.js";
...
// The greying rule is one relation read from either side.
const FEATURE_DOMINANCE = "A set stands out inside a feature only when its |held-out r| beats the mean of the"
  + " feature's other effects above 0.10 by a further 0.10, weighed over every set the feature reproduces. That mean"
  + " is taken once per feature, over everything but its strongest effect, so several sets can clear it together."
  + " Those keep their colour, the rest are greyed, and a feature whose evidence singles out nothing greys entirely.";

const FEATURE_UNLISTED = "A feature is credited with the sets it points at only where the site lists the feature at"
...
const FEATURE_CUTS = (limit) => `Cards are ranked by |held-out r|. The list always holds the strongest ${limit}`
  ...
  + ` is built around remains visible either way; every other grey card is hidden.`;
...
export const COPY = {
  ...
  byBlock: {
    ...
    validDot: "The held-out evidence singles out at least one of this feature's sets and that set has a card here,"
      + " so the feature carries a card in colour. Features without one come after these in this order.",
  ...
  genes: {
    featureHeading: "Genes clustered in this feature",
    pathwayHeading: "Genes active when this pathway is above baseline",
    featureInfo: "Each chip carries two numbers over the tiles where the gene is measured. I is Moran's I of the gene"
      + " over the selected feature's manifold, high when tiles that agree on the gene sit together, and the cards are"
      + " ranked by it. r is the gene's correlation with the feature norm.",
    pathwayInfo: "These genes are selected independently of the feature cards. They are ranked by how often they are"
      + " detected on tiles where the pathway score is positive. % is that detection rate; r is correlation with the"
      + " aggregate pathway score and is descriptive because the gene contributes to that score.",
    featureTitle: (gene, label) =>
      `${gene.symbol} · clustering ${gene.clustering}, Moran's I over the ${gene.n_tiles} tiles of ${label} where it`
    ...
    pathwayTitle: (gene, name) =>
      `${gene.symbol} · detected on ${gene.n_detected_tiles.toLocaleString()} of the ${gene.n_tiles.toLocaleString()}`
      + ` tiles where ${name} is above its training baseline (${Math.round(gene.detection * 100)}%) · correlation with`
      + ` the aggregate pathway score r ${gene.pathway_r} · mean ${gene.mean} log1p CPM, sd ${gene.std} · measured on`
      + ` ${gene.train_slides}/46 training and ${gene.heldout_slides}/12 held-out slides · click to use pathway-ranked`
      + " tiles and overlay this gene",
    ...
    pathwayDetectionLabel: "%: share of positive pathway-score tiles where this gene is detected. Cards are ranked by it.",
    pathwayCorrelationLabel: "r: Pearson correlation between this gene's expression and the aggregate pathway score."
      + " The gene contributes to that score, so this describes contribution rather than independent evidence.",
  },
  ...
  blocks: {
    ...
    panel: (dominance) => `<span><strong>held-out r:</strong> ${FEATURE_HELDOUT_R}</span>
      ...
      ${dominance.n_blocks_scored.toLocaleString()} features that reproduce anything single out at least one set, and
      ${dominance.n_dominant} of those are features the site lists, carrying ${dominance.n_lit} cards in colour
      between them.</span>`,
  },
  ...
  tile: {
    ...
    gene: {
      ...
      subtitle: (label) => `tiles of ${label}, coloured by mean log1p CPM`,
    ...
    pathway: {
      ...
      subtitle: (label, pathwayId) => `tiles of ${label}, coloured by ${pathwayId} score`,
  ...
  cross: {
    ...
    marked: (label, where) => ` The magenta ring marks <b>${label}</b>, ${where}.`,
    where: { pathway: "selected in the cards above", block: "the feature this view is built around" },
    unmarked: (label, pathwayId, blocks) => ` Nothing is ringed: <b>${label}</b> transfers on ${pathwayId} but
    ...
    strongest: {
      ...
      note: "Each feature is coloured by the association reproduced on held-out tiles with the largest absolute effect"
        + " there. This is the feature's leader rather than a set it points at, since no margin is required: a feature"
