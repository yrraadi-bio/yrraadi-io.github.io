import { GENE_STOPS, blockLabel, blockNumber, el, loadJson, state, subset } from "./core.js";
...
export async function loadManifold() {
  ...
  state.manifold.pathways = new Set((await loadJson(`data/${state.slug}/manifold/pathways_index.json`)).pathways.map((pathway) => pathway.pathway_id));
}
...
export async function loadTilePathway(pathwayId) {
...
export async function loadPathwayCoords(pathwayId) {
...
export async function renderBlockManifold(geneAvailable) {
...
function blockHover(indices) {
  ...
  return indices.map((index) => {
    const code = state.manifold.labels.code[index];
    const strongest = code >= 0 ? legend[code].name
      : code === -2 ? COPY.hover.strongestOther : COPY.hover.strongestNone;

    return COPY.hover.block(blockNumber(blocks.layer[index], blocks.block[index]), strongest);
  });
}
